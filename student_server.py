"""
Student RAG Server v3.0 — Hệ thống RAG tối ưu cho Competition
MSSV: B22DCVT415
...
LOCAL_MODEL_DIR = os.path.join(BASE_DIR, "models", "vietnamese-sbert")
...
# ============================================================
# VECTOR STORE v3 — Parent-Child + Persistence
# ============================================================
class VectorStore:
    """
...
# KHỞI TẠO
# ============================================================
app = FastAPI(title="Student RAG Server v3", version="3.0")

print("=" * 60)
print(f"[INIT] Student RAG Server v3.0 — MSSV: {STUDENT_ID}")
...
_ = embedding_model.encode(["warm up"], convert_to_numpy=True)
...
async def upload_document(req: UploadRequest):
    ...
    try:
        ...
        print(f"[UPLOAD] ⏳ 2/3 Embedding {len(child_chunks)} children...")
        child_embeddings = embedding_model.encode(
            child_chunks, show_progress_bar=True,
...
if __name__ == "__main__":
    print(f"\n{'='*60}")
    print(f"  🚀 STUDENT RAG SERVER v3.0")
    ...
    print(f"  ⚡ Strategies: Parent-Child | BM25+Vector+RRF | MMR | Reorder | CoT")
